<template>
    <div class="body" ref="scrollRef">
        <div v-if="!conversationList.length" class="explain">
            <img class="logo" alt="Vue logo" :src="logoIcon"/>
            <div class="expositoryCase" style="white-space: pre-wrap;">{{ data.botInitialization }}</div>
            <div class="consume">
                <el-icon>
                    <Goods/>
                </el-icon>
            ...
            "
                ...
                <div class="answer">
                    <el-avatar
                            ...
                            :size="35"
                            :icon="UserFilled"
                            :src="require('../assets/avatars/'+data.icon)"
                    />
        ...
        <div class="footer">
            <div class="footer-bar">
                ...
                >
                    <div style="padding-top: 4px">
                        <el-icon size="13px" style="padding-right: 3px">
                            <!-- 选择一个适合的图标，这里使用的是上传图标，你可以根据需求更改 -->
                            <Upload/>
                        </el-icon>
                    </div>
                    <div :style="{ color: file_url !== '' ? '#6DB4FD' : 'inherit' }">
                        {{ file_url !== '' ? '已上传:' + fileName : '上传文件' }}
                    </div>
                </el-upload>
            ...
            <div class="cache-content">
                <div class="cache-scrollbar">
                    <el-scrollbar height="250px">
                        <div
                                class="cache-padding"
                                v-for="(item, index) in GPTsCache.array"
                                :key="index"
                        >
...
import {FavoritesAdd, fileUpload, GetUserInfo} from "../../api/BSideApi";
...
export default {
    ...
    computed: {
        ...
        logoIcon() {
            return this.data.icon && require('../assets/avatars/' + this.data.icon)
        }
    },
    setup() {
        let data = ref({});
        ...
        onMounted(() => {
            ...
            let modelInfo = localStorage.getItem("gptsData");
            data.value = JSON.parse(modelInfo);
            GPTsCacheName = data.value.gptsModel + "Cache"
            //获取对话缓存数据
            let item = localStorage.getItem(GPTsCacheName);
            if (store.getters.userinfo) {
                if (!store.getters.userinfo) return (loginVisible.value = true);
                if (item) {
                    GPTsCache.value = JSON.parse(item);
                    let value = GPTsCache.value;
                    ...
                    localStorage.setItem(
                        GPTsCacheName,
                        JSON.stringify(GPTsCache.value)
                    );
                }
            }
        });
        ...
        function switchChat(index) {
            GPTsCache.value.index = index;
            conversationList.value = GPTsCache.value.array[index].context;
            localStorage.setItem(
                GPTsCacheName,
                JSON.stringify(GPTsCache.value)
            );
        ...
        function clearDialogue(index) {
            if (index !== GPTsCache.value.index) {
                let i = parseInt(GPTsCache.value.index);
                if (index < i) {
                    GPTsCache.value.index = i - 1;
                }
                GPTsCache.value.array.splice(index, 1);
            }
            localStorage.setItem(
                GPTsCacheName,
                JSON.stringify(GPTsCache.value)
        ...
        function writeDialogue() {
            ...
            let value = GPTsCache.value;
            GPTsCache.value.array[value.index].time = Date.now();
            if (item.length > 0) {
                GPTsCache.value.array[value.index].title = item[
                item.length - 1
                    ].user
                    .trim()
                    .slice(0, 25);
            }
            GPTsCache.value.array[value.index].context = item;
            localStorage.setItem(
                GPTsCacheName,
                JSON.stringify(GPTsCache.value)
            );
        ...
        function createdNewChat() {
            GPTsCache.value.array.unshift({
                title: "新对话",
                time: Date.now(),
                context: [],
            });
            GPTsCache.value.index = 0;
