...
public class GptServiceImpl implements GptService {
    ...
    @Override
    public Flux<String> concatenationDiyGpt(final GptModel model, final String modelName, final boolean isAdvanced, final ChatGptCommon.ChatGptStructure chatGptStructure) {
        //设置请求模型名称
    ...
    //GPT识图模型
    @Override
    public Flux<String> concatenationVisionGpt(final GptVisionModel gptVisionModel, final String modelName, final boolean isAdvanced, final ChatGptCommon.ChatGptStructure chatGptStructure) {
        //设置请求模型名称
        gptVisionModel.setModel(modelName).setMax_tokens(300);
        return webClient.baseUrl(isAdvanced ? chatGptStructure.getOpenAiPlusUrl() : chatGptStructure.getOpenAiUrl())
                .defaultHeader(HttpHeaders.AUTHORIZATION, "Bearer " + (isAdvanced ? chatGptStructure.getOpenPlusKey() : chatGptStructure.getOpenKey())).build()
                .post()
                .uri(ServerConstant.GPT_DIALOGUE)
                .body(BodyInserters.fromValue(gptVisionModel))
                .retrieve()
                .bodyToFlux(String.class);
    }
    ...
    //fastgpt
    @Override
    public Flux<String> concatenationFastGpt(final GptModel model, final String modelName, final ChatGptCommon.ChatGptStructure chatGptStructure) {
        //设置请求模型名称
        model.setModel(modelName);
        String ApiKey = switch (modelName) {
            case "法律助手" -> "Bearer fastgpt-Ze05alNGks1aHhcptRbfjgjbVm";
            default -> null;
        };
        return webClient.baseUrl("http://127.0.0.1:3000/api/v1")
