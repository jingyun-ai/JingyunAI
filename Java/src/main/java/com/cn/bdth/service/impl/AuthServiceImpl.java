...
import com.cn.bdth.utils.AliSendSmsUtils;
...
public class AuthServiceImpl implements AuthService {
    ...
    private final AliSendSmsUtils aliSendSmsUtils;
    ...
    @Override
    public void phoneBack(final PhoneCodeDto dto) {
        final String code = dto.getCode();
        final String KEY = AuthConstant.PHONE_CODE + dto.getPhone();
        final Object value = redisUtils.getValue(KEY);
        if (value != null && value.equals(code)) {
            final Long l = userMapper.selectCount(new QueryWrapper<User>()
                    .lambda().eq(User::getPhone, dto.getPhone())
                    .select(User::getUserId));
            if (l <= 0) {
                throw new EmailBackException(ExceptionMessages.PHONE_NOT_EXIST);
            }
            userMapper.update(new User()
                    .setPassword(SaSecureUtil.md5BySalt(dto.getPassword(), SALT)), new QueryWrapper<User>().lambda().eq(User::getPhone, dto.getPhone()));
            redisUtils.delKey(KEY);
        } else {
            throw new EmailBackException(ExceptionMessages.CODE_ERR);
        }
    ...
    @Override
    public void phoneEnroll(final PhoneCodeDto dto) {
        final String code = dto.getCode();
        final String KEY = AuthConstant.PHONE_CODE + dto.getPhone();
        final Object value = redisUtils.getValue(KEY);
        if (value != null && value.equals(code)) {
    ...
    @Override
    public void getPhoneEnrollCode(final String phone) {
        final String code = RandomStringUtils.random(4, false, true);
        try {
            redisUtils.setValueTimeout(AuthConstant.PHONE_CODE + phone, code, 300);
            aliSendSmsUtils.send(phone,code);
        }catch (Exception e) {
            throw new RegistrationException(ExceptionMessages.VERIFICATION_CODE_ERR, 500);
        }
    }
    ...
    @Override
    public String phoneLogin(final PhoneLoginDto dto) {
        final User user = userMapper.selectOne(new QueryWrapper<User>()
                .lambda()
        ...
        if (user == null) {
            throw new LoginPasswordException(ExceptionMessages.PHONE_LOGIN_PWD_ERR);
        }
        StpUtil.login(user.getUserId());
        //设置具体TOKEN Session权限
        StpUtil.getSession()
