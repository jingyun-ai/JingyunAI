...
public class AliUploadUtils {
    ...
    private String getContentTypeWithTika(MultipartFile file) {
        try {
            Tika tika = new Tika();
            return tika.detect(file.getInputStream());
        } catch (IOException e) {
            log.error("使用 Apache Tika 获取文件ContentType时发生错误： {}", e.getMessage());
            return "application/octet-stream"; // 发生错误时默认为二进制流
        }
    }
    ...
    public String uploadFile(final MultipartFile file, final String path, final String newFileName, final boolean isImage) {
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            if (isImage) {
                ...
                objectMetadata.setContentType(getContentTypeWithTika(file));
