...
public class AuthController {
    ...
    @GetMapping(value = "/phone/code/{phone}", name = "网页获取手机验证码", produces = MediaType.APPLICATION_JSON_VALUE)
    public Result getPhoneLoginCode(@PathVariable final String phone) {
        try {
            authService.getPhoneEnrollCode(phone);
            return Result.ok();
        } catch (RegistrationException e) {
            return Result.error(e.getMessage());
        }
     ...
     * @return the result
     */
    @PostMapping(value = "/phone/password/back", name = "找回密码", produces = MediaType.APPLICATION_JSON_VALUE)
    public Result RetrievePhonePassword(@RequestBody @Validated final PhoneCodeDto dto) {
        try {
            authService.phoneBack(dto);
            return Result.ok();
        } catch (EmailBackException e) {
            return Result.error(e.getMessage());
        }
    ...
    @PostMapping(value = "/phone/enroll", name = "注册账号", produces = MediaType.APPLICATION_JSON_VALUE)
    public Result phoneEnroll(@RequestBody @Validated final PhoneCodeDto dto) {
        try {
            authService.phoneEnroll(dto);
            return Result.ok();
        } catch (RegistrationException e) {
            return Result.error(e.getMessage());
        }
    ...
    @PostMapping(value = "/phone/login", name = "登录", produces = MediaType.APPLICATION_JSON_VALUE)
    public Result phoneLogin(@RequestBody @Validated final PhoneLoginDto dto) {
        try {
            return Result.data(authService.phoneLogin(dto));
        } catch (LoginPasswordException e) {
            return Result.error(e.getMessage());
        }
    }
