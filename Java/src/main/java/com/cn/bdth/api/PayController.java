...
public class PayController {
     ...
     * @return the result
     */
    @PostMapping(value = "/alipay/page/{productId}", name = "支付宝电脑网站支付", produces = MediaType.APPLICATION_JSON_VALUE)
    public Result alipayPagePay(@PathVariable final Long productId) {
        try {
            return Result.data(payService.generatePagePay(productId));
        } catch (OrdersException e) {
            return Result.error(e.getMessage(), e.getCode());
        }
    }
     ...
     * @return the result
     */
    @PostMapping(value = "/weixin/pay/{productId}", name = "微信支付", produces = MediaType.APPLICATION_JSON_VALUE)
    public Result weixinPayQrCode(@PathVariable final Long productId) {
        try {
            return Result.data(payService.generateWeixinPayQrCode(productId));
        } catch (OrdersException e) {
            return Result.error(e.getMessage(), e.getCode());
        }
    }
     ...
     * @return the product list
     */
    @GetMapping(value = "/orders/page", name = "获取我的充值订单记录", produces = MediaType.APPLICATION_JSON_VALUE)
